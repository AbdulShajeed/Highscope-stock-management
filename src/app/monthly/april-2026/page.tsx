...
export default function April2026Page() {
  ...
  const key = 'April-2026'

  useEffect(() => {
    const fetchData = async () => {
      if (cache[key] && Date.now() - cache[key].timestamp < CACHE_TTL) {
        setReport(cache[key].data); setLoading(false)
        fetch('/api/monthly-report?month=April&year=2026').then(r => r.json()).then(d => { cache[key] = { data: d, timestamp: Date.now() }; setReport(d) }).catch(() => {})
        return
      }
      try {
        const res = await fetch('/api/monthly-report?month=April&year=2026')
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-bold text-gray-900">April 2026 - Monthly Stock Report</h1>
              <p className="mt-2 text-sm text-gray-600">Stock summary across all categories for April 2026</p>
            </div>
            <div className="flex space-x-2">
              <Link href="/monthly/may-2026" className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors">Next Month (May) →</Link>
            </div>
          </div>
        </div>
      </header>
      <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 flex justify-between">
          <div />
          <Link href="/monthly/may-2026" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">Next Month (May) →</Link>
        </div>
      </main>
    </div>
  )
