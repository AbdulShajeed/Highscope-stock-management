...
interface ReportData {
  month: string; year: number; categories: any[]; totals: any
}
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (cache[key] && Date.now() - cache[key].timestamp < CACHE_TTL) {
        ...
        fetch('/api/monthly-report?month=June&year=2026').then(r => r.json()).then(d => { cache[key] = { data: d, timestamp: Date.now() }; setReport(d) }).catch(() => {})
        return
      }
      try {
        const res = await fetch('/api/monthly-report?month=June&year=2026')
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-gray-600">Stock summary across all categories for June 2026</p>
            </div>
            <div className="flex space-x-2">
              <Link href="/monthly/may-2026" className="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors">← Previous Month (May)</Link>
            </div>
          </div>
        </div>
      </header>
      <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 flex justify-between">
          <Link href="/monthly/may-2026" className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors">← Previous Month (May)</Link>
          <div className="text-gray-500 text-sm">Current Month - June 2026</div>
        </div>
      </main>
    </div>
  )
