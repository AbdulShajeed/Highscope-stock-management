...
export default function May2026Page() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('/api/monthly-report?month=May&year=2026')
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-gray-600">
                Stock summary across all categories for May 2026
              </p>
            </div>
            <div className="flex space-x-2">
              <Link
                href="/monthly/april-2026"
                className="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors"
              >
                ← Previous Month (April)
              </Link>
              <Link
                href="/monthly/june-2026"
